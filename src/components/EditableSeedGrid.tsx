...
export default function EditableSeedGrid({
  ...
  const handleAddSeed = async (values: AddSeedForm, files: File[]) => {
    ...
    // Require a received date so SKU can be generated by your inventory->SKU trigger
    ...
    // Optional: send pricing defaults, or null
    ...
    // 3) Upload images (same as your current code)
    for (const file of files) {
      ...
      await supabase.from('seed_images').insert({ seed_id: newSeedId, image_path: path });
    }

    // 4) Refresh UI: fetch the new seed to include SKU generated by trigger
